...
import { RDO_CREATION_CONTEXT_INCOMPATIBLE } from "./rdoLookupApi";
...
beforeEach(() => {
  ...
  mocks.createPendingDraft.mockResolvedValue({
    draft: { ...draft, syncStatus: "LOCAL_PENDING" },
    mutation: {},
  });
  setOnline(false);
});
...
function renderDialog(overrides: {
  onClose?: () => void;
  onCreated?: (
    draft: ReturnType<typeof createEmptyRdo>,
...
describe("diálogo obra-first de RDO", () => {
  ...
  it("remove o hard stop legado e cria rascunho explicitamente LOCAL_PENDING", async () => {
    ...
    await waitFor(() =>
      expect(mocks.createPendingDraft).toHaveBeenCalledWith(
        localPendingContext(),
      ),
    );
  ...
  it("refaz a validação estrita do contexto pelo botão acessível de tentativa", async () => {
    ...
    mocks.requireContext
      ...
      .mockResolvedValueOnce({
        kind: "CANONICAL",
        source: "SERVER",
        cachedAt: "2026-07-22T12:02:00.000Z",
        context: context(),
      });
    renderDialog();
    ...
    expect(await screen.findByText("Servidor")).toBeVisible();
