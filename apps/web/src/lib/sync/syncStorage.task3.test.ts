...
describe("atomic canonical push results", () => {
  it("does not leave a corrupted canonical RDO or its dependents SYNCING after quarantine", async () => {
    ...
    const corrupted = {
      ...committed.mutation,
      trace: {
        ...committed.mutation.trace,
        payloadHash: "0".repeat(64),
      },
    };
    await database.put("outbox_mutations", corrupted);

    const locked = await markMutationAsSyncing(corrupted);
    await expect(toPushMutationRequest(locked)).rejects.toThrow(
      "payload hash",
    );
    await rejectMutationLocally(
      corrupted.clientMutationId,
      "LOCAL_CANONICAL_INVALID",
      "Canonical mutation payload hash is incoherent.",
    );
    ...
    expect(
      ...
      status: "REJECTED",
      blockedReason: "LOCAL_CANONICAL_INVALID",
    });
    expect(await database.get("rdos", RDO_ID)).toMatchObject({
      syncStatus: "ERROR",
    });
    expect(
      await database.get("rdoMaoObra", "mao-corrupt-canonical"),
    ).toMatchObject({
      syncStatus: "ERROR",
    });
    expect(
