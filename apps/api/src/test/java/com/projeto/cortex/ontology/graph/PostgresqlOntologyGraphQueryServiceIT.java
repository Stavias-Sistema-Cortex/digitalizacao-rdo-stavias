...
class PostgresqlOntologyGraphQueryServiceIT {
    ...
    void resolvesWorksiteAndQueriesProjectedJsonbWithBoundedTraversal() {
        try (PostgreSQLContainer<?> database = new PostgreSQLContainer<>("postgres:18")
            ...
            OntologyGraphPostgresqlTestSupport.insertCanonicalSource(
                    jdbc,
                    initialBatch.commitSequence(),
                    initialBatch.commitId(),
                    initialBatch.sourceWorksiteId()
            );
            ...
            jdbc.update("""
                    INSERT INTO operational_states (
                        id, entity_id, state_type, state_value, valid_from,
                        metadata_json, source_worksite_id
    ...
    void keepsExclusiveRdoScopeWhenProjectedActorsAndAssetsAreSharedAcrossWorksites()
        ...
        try (PostgreSQLContainer<?> database = new PostgreSQLContainer<>("postgres:18")
            ...
            topology.forEach(batch ->
                    OntologyGraphPostgresqlTestSupport.insertCanonicalSource(
                            jdbc,
                            batch.commitSequence(),
                            batch.commitId(),
                            batch.sourceWorksiteId()
                    )
            );
    ...
    @Test
    void sharedEntityKeepsPerWorksiteMetadataEventsStatesAndEvidenceOverHttp()
            throws Exception {
        try (PostgreSQLContainer<?> database = new PostgreSQLContainer<>("postgres:18")
                .withDatabaseName("StaviasCortex")) {
            database.start();
            ...
            jdbc.update("""
                    ...
                    WHERE entity_id = ?
                    """, WORKSITE_A, WORKSITE_B, collaborator);
            jdbc.update("""
                    INSERT INTO operational_evidences (
                        id, entity_id, evidence_type, source_type, source_id,
                        description, metadata_json, created_at, source_worksite_id
                    ) VALUES
                        ('evidence-shared-a', ?, 'BOUNDARY', 'test', 'source-a',
                         'Evidence A', jsonb_build_object('worksiteId', ?),
                         timestamp '2026-07-21 12:10:00', ?),
                        ('evidence-shared-b', ?, 'BOUNDARY', 'test', 'source-b',
                         'Evidence B', jsonb_build_object('worksiteId', ?),
                         timestamp '2026-07-21 12:20:00', ?)
                    """,
                    collaborator, WORKSITE_A, WORKSITE_A,
                    collaborator, WORKSITE_B, WORKSITE_B
            );

            OntologyGraphQueryService queryService = queryService(jdbc);
            MockMvc betaA = scopedMockMvc(queryService, "beta-a", WORKSITE_A);
            MockMvc betaB = scopedMockMvc(queryService, "beta-b", WORKSITE_B);
            ObjectMapper json = new ObjectMapper();

            JsonNode entityA = responseJson(json, betaA.perform(
                    get("/api/ontology/entities/{id}", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-a")
            ).andReturn());
            JsonNode eventsA = responseJson(json, betaA.perform(
                    get("/api/ontology/entities/{id}/events", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-a")
            ).andReturn());
            JsonNode statesA = responseJson(json, betaA.perform(
                    get("/api/ontology/entities/{id}/states", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-a")
            ).andReturn());
            JsonNode evidencesA = responseJson(json, betaA.perform(
                    get("/api/ontology/entities/{id}/evidences", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-a")
            ).andReturn());

            JsonNode entityB = responseJson(json, betaB.perform(
                    get("/api/ontology/entities/{id}", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-b")
            ).andReturn());
            JsonNode eventsB = responseJson(json, betaB.perform(
                    get("/api/ontology/entities/{id}/events", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-b")
            ).andReturn());
            JsonNode statesB = responseJson(json, betaB.perform(
                    get("/api/ontology/entities/{id}/states", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-b")
            ).andReturn());
            JsonNode evidencesB = responseJson(json, betaB.perform(
                    get("/api/ontology/entities/{id}/evidences", collaborator)
                            .requestAttr(CurrentUserService.REQUEST_ATTRIBUTE_USER_ID, "beta-b")
            ).andReturn());

            SoftAssertions.assertSoftly(softly -> {
                softly.assertThat(entityA.at("/metadata/worksiteId").asText())
                        .isEqualTo(WORKSITE_A);
                softly.assertThat(eventsA).hasSize(1);
                softly.assertThat(eventsA.at("/0/payload/worksiteId").asText())
                        .isEqualTo(WORKSITE_A);
                softly.assertThat(statesA).hasSize(1);
                softly.assertThat(statesA.at("/0/stateValue").asText())
                        .isEqualTo("ACTIVE_A");
                softly.assertThat(evidencesA).hasSize(1);
                softly.assertThat(evidencesA.at("/0/metadata/worksiteId").asText())
                        .isEqualTo(WORKSITE_A);

                softly.assertThat(entityB.at("/metadata/worksiteId").asText())
                        .isEqualTo(WORKSITE_B);
                softly.assertThat(eventsB).hasSize(1);
                softly.assertThat(eventsB.at("/0/payload/worksiteId").asText())
                        .isEqualTo(WORKSITE_B);
                softly.assertThat(statesB).hasSize(1);
                softly.assertThat(statesB.at("/0/stateValue").asText())
                        .isEqualTo("ACTIVE_B");
                softly.assertThat(evidencesB).hasSize(1);
                softly.assertThat(evidencesB.at("/0/metadata/worksiteId").asText())
                        .isEqualTo(WORKSITE_B);
            });
        }
    }
    ...
    private static MockMvc scopedMockMvc(
        ...
        when(currentUserService.allowedObraIds(userId))
    ...
    private static JsonNode responseJson(ObjectMapper objectMapper, MvcResult result)
            throws Exception {
    ...
    private static CommittedOperationalEvent projectedParticipation(
        ...
        return projectedEvent(
                ...
                Map.of(
                        ...
                        "collaboratorName", "Compartilhado",
                        "status", WORKSITE_A.equals(worksiteId) ? "ACTIVE_A" : "ACTIVE_B"
                ),
                worksiteId
        );
    }
    ...
    private static void seedSharedTopology(JdbcTemplate jdbc) {
        entity(jdbc, "worksite-a", "WORKSITE", WORKSITE_A, "{}", "Obra A", WORKSITE_A);
        entity(jdbc, "worksite-b", "WORKSITE", WORKSITE_B, "{}", "Obra B", WORKSITE_B);
        entity(jdbc, "worksite-c", "WORKSITE", WORKSITE_C, "{}", "Obra C", WORKSITE_C);
        entity(
                jdbc,
                "shared",
                "COLLABORATOR",
                "shared",
                jsonWorksite(WORKSITE_B),
                "Compartilhado",
                null
        );
        entity(jdbc, "local", "RDO", "local", jsonWorksite(WORKSITE_A), "Local", WORKSITE_A);
        entity(
                jdbc,
                "foreign",
                "RDO",
        ...
        entity(jdbc, "orphan", "RDO", "orphan", "{}", "Órfão", null);
        entity(jdbc, "cycle-1", "ACTIVITY", "cycle-1", "{}", "Ciclo 1", null);
        entity(jdbc, "cycle-2", "ACTIVITY", "cycle-2", "{}", "Ciclo 2", null);
        entity(
                jdbc,
                "literal-search",
                "RDO",
                "literal-search",
                jsonWorksite(WORKSITE_A),
                "Literal %_\\",
                WORKSITE_A
        );
        entity(
    ...
    private static void seedScopedObjects(JdbcTemplate jdbc) {
        ...
        event(jdbc, "event-local", "shared", "local", 1, WORKSITE_A);
        event(jdbc, "event-orphan", "shared", "orphan", 3, null);
        event(jdbc, "event-foreign", "shared", "foreign", 4, WORKSITE_B);
        state(jdbc, "state-local", "local", WORKSITE_A);
        state(jdbc, "state-foreign", "foreign", WORKSITE_B);
        state(jdbc, "state-orphan", "orphan", null);
        evidence(jdbc, "evidence-local", "local", WORKSITE_A);
        evidence(jdbc, "evidence-foreign", "foreign", WORKSITE_B);
        evidence(jdbc, "evidence-orphan", "orphan", null);
    }
    ...
    private static void entity(
        ...
        if (sourceWorksiteId != null) {
            jdbc.update("""
                    INSERT INTO ontology_entity_worksite_snapshots (
                        entity_id, source_worksite_id, canonical_name, metadata_json
                    ) VALUES (?, ?, ?, ?::jsonb)
                    """, id, sourceWorksiteId, name, metadata);
        }
    }
    ...
    private static void event(
        ...
        jdbc.update("""
                INSERT INTO ontology_events (
                    ...
                    description, occurred_at, source_worksite_id
                ) VALUES (?, 'BOUNDARY', ?, ?, 'test', ?, 'Evento',
                    timestamp '2026-07-21 12:00:00' + (? * interval '1 minute'), ?)
                """, id, entity, related, id, minute, sourceWorksiteId);
    }
    ...
    private static void state(
        ...
        jdbc.update("""
                INSERT INTO operational_states (
                    id, entity_id, state_type, state_value, source_worksite_id
                ) VALUES (?, ?, 'BOUNDARY', 'VISIBLE', ?)
                """, id, entity, sourceWorksiteId);
    }
    ...
    private static void evidence(
        ...
        jdbc.update("""
                INSERT INTO operational_evidences (
                    id, entity_id, evidence_type, source_type, source_id,
                    description, source_worksite_id
                ) VALUES (?, ?, 'BOUNDARY', 'test', ?, 'Evidência', ?)
                """, id, entity, id, sourceWorksiteId);
    }
