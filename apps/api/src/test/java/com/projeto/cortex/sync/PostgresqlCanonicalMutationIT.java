...
class PostgresqlCanonicalMutationIT {
    ...
    @Test
    void canonicalLinkPreservesServerAuthoredAuditStateAndRelations() throws Exception {
        Setup setup = setup(false);
        SyncPushRequest.MutacaoCliente original = canonicalMutation(
                setup,
                setup.deviceOne(),
        ...
        ObjectNode forgedBefore = mapper.createObjectNode();
        forgedBefore.put("value", "client-forged-before");
        SyncPushRequest.MutacaoCliente crafted = withRelatedEntities(
                copyCanonicalWithPatch(
                        original,
                        original.payload().deepCopy(),
                        original.trace().payloadHash(),
                ...
                List.of(new SyncPushRequest.RelatedEntity(
                        "OBRA",
                        setup.obraId(),
                        "client-forged-relation"
                ))
        );

        SyncPushResponse response = setup.service().push(
                new SyncPushRequest(setup.deviceOne(), List.of(crafted))
        );

        assertThat(response.resultados()).singleElement().satisfies(result -> {
            assertThat(result.status()).isEqualTo("APLICADA");
            assertThat(result.resultado().path("serverAssigned").asText())
                    .isEqualTo("server-after");
        });

        Map<String, Object> audit = jdbc.queryForMap(
                """
                SELECT estado_anterior_json::text AS before_state,
                       estado_novo_json::text AS after_state,
                       entidades_relacionadas_json::text AS related_entities,
                       payload_json::text AS event_payload,
                       fonte,
                       tipo_evento
                FROM cortex_evento_operacional
                WHERE commit_seq = ?
                """,
                response.resultados().getFirst().eventoServidorCommitSeq()
        );
        JsonNode beforeState = mapper.readTree((String) audit.get("before_state"));
        JsonNode afterState = mapper.readTree((String) audit.get("after_state"));
        JsonNode relatedEntities = mapper.readTree(
                (String) audit.get("related_entities")
        );
        JsonNode eventPayload = mapper.readTree((String) audit.get("event_payload"));

        assertThat(beforeState.path("authoritativeBefore").asText())
                .isEqualTo("server-before");
        assertThat(beforeState.has("value")).isFalse();
        assertThat(afterState.path("serverAssigned").asText())
                .isEqualTo("server-after");
        assertThat(afterState.path("value").asText())
                .isEqualTo("server-normalized");
        assertThat(relatedEntities).singleElement().satisfies(relation -> {
            assertThat(relation.path("tipo").asText()).isEqualTo("OBRA");
            assertThat(relation.path("id").asText()).isEqualTo(setup.obraId());
            assertThat(relation.path("nome").asText())
                    .isEqualTo("server-authored-relation");
        });
        assertThat(eventPayload.path("serverPayloadMarker").asText())
                .isEqualTo("server-authored");
        assertThat(audit).containsEntry("fonte", "POSTGRESQL_IT")
                .containsEntry("tipo_evento", "RDO_CRIADO");
    }

    @Test
    void canonicalLinkPersistsExactAuthoritativeRelationsFromAppliedContract()
            throws Exception {
        Setup setup = setup(false, null, true);
        SyncPushRequest.MutacaoCliente original = canonicalMutation(
                setup,
                setup.deviceOne(),
                UUID.randomUUID().toString(),
        ...
        JsonNode related = mapper.readTree((String) audit.get("related_entities"));

        assertThat(related).singleElement().satisfies(relation -> {
            assertThat(relation.path("tipo").asText()).isEqualTo("OBRA");
            assertThat(relation.path("id").asText()).isEqualTo(setup.obraId());
            assertThat(relation.has("nome")).isFalse();
        });
        assertThat(mapper.readTree((String) audit.get("before_state"))
                .path("authoritativeBefore").asText()).isEqualTo("server-before");
        assertThat(mapper.readTree((String) audit.get("after_state"))
                .path("serverAssigned").asText()).isEqualTo("server-after");
        assertThat(audit).containsEntry("tipo_evento", "RDO_CRIADO");
    }
    ...
    private Setup setup(boolean failOnce, String appliedEntityOverride) {
        return setup(failOnce, appliedEntityOverride, false);
    }

    private Setup setup(
            boolean failOnce,
            String appliedEntityOverride,
            boolean authoritativeEvent
    ) {
        ...
        SyncOperationHandler handler = new SyncOperationHandler() {
            ...
            public AppliedSyncMutation apply(
                ...
                memory.registrarEventoAuditado(
                        ...
                        13,
                        Map.of(
                                "value", mutation.payload().path("value").asText(),
                                "serverPayloadMarker", "server-authored"
                        ),
                        ownerId,
                        context.deviceId(),
                        mutation.trace() == null
                        ...
                        Map.of("authoritativeBefore", "server-before"),
                        Map.of(
                                "serverAssigned", "server-after",
                                "value", "server-normalized"
                        ),
                        "SUCESSO",
                        null
                );
                ...
                result.put("serverAssigned", "server-after");
                if (authoritativeEvent) {
                    return new AppliedSyncMutation(
                            ...
                            new AppliedSyncMutation.AuthoritativeEvent(
                                    "RDO_CRIADO",
                                    List.of(
                                            new AppliedSyncMutation
                                                    .AuthoritativeRelatedEntity(
                                                    "OBRA", obraId
                                            )
                                    )
                            )
    ...
    private SyncPushRequest.MutacaoCliente withRelatedEntities(
            SyncPushRequest.MutacaoCliente original,
            List<SyncPushRequest.RelatedEntity> relatedEntities
    ) {
        return new SyncPushRequest.MutacaoCliente(
                original.clientMutationId(), original.entidadeTipo(),
                original.entidadeId(), original.operacao(), original.baseVersao(),
                ...
                original.baseVersion(), original.changedFields(),
                original.occurredAt(), original.trace(), original.fieldPatch(),
                relatedEntities, original.dependsOnMutationIds()
        );
    }
