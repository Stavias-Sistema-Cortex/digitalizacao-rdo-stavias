...
/** MySQL row-lock implementation shared safely by every API instance. */
@Repository
public class RateLimitBucketRepository {
    ...
    /**
     * Non-mutating circuit-breaker probe. A later {@link #consume} remains the
     * atomic authority, so a concurrent race can only fail closed.
     */
    public boolean hasCapacity(
        ...
        if (rows.size() > 1) {
            throw new IllegalStateException(
                    "Rate limit compartilhado ambíguo."
            );
        }
        if (rows.isEmpty()) {
            return true;
        ...
        if (!bucket.now().isBefore(
                bucket.windowStartedAt().plusSeconds(windowSeconds)
        )) {
            return true;
        }
    ...
    public boolean consume(
        ...
        if (locked.size() != keys.size()) {
            throw new IllegalStateException(
                    "Rate limit compartilhado indisponível."
            );
        }

        boolean allowed = true;
        for (Bucket bucket : locked) {
            allowed &= consumeLocked(
                    bucket,
                    maxRequests,
                    windowSeconds
            );
    ...
    private boolean consumeLocked(
        ...
        Instant windowEnd = bucket.windowStartedAt()
        ...
        if (bucket.blockedUntil() != null
                && bucket.now().isBefore(bucket.blockedUntil())) {
            updateBucket(
                    bucket.key(),
                    bucket.windowStartedAt(),
                    Math.min(bucket.counter() + 1, maxRequests + 1),
                    bucket.blockedUntil()
            );
            return false;
        }
    ...
    private List<String> normalizeKeys(List<String> bucketKeys) {
        if (bucketKeys == null) {
            throw invalidKeys();
        }
    ...
    private void validatePolicy(int maxRequests, int windowSeconds) {
        if (maxRequests < 1
                || maxRequests > 100_000
                || windowSeconds < 60
                || windowSeconds > 3_600) {
            throw new IllegalArgumentException(
                    "Política de rate limit inválida."
            );
        }
    }

    private IllegalArgumentException invalidKeys() {
        return new IllegalArgumentException(
                "Buckets de rate limit inválidos."
        );
    }
