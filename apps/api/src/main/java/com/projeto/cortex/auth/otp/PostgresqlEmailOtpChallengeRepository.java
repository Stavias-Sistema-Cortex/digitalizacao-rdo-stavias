...
public class PostgresqlEmailOtpChallengeRepository
    ...
    public PostgresqlEmailOtpChallengeRepository(JdbcTemplate jdbcTemplate) {
    ...
    public Optional<DeliveryState> deliveryState(String challengeId) {
        List<DeliveryState> rows = jdbcTemplate.query("""
                SELECT status, expira_em, clock_timestamp() AS agora
    ...
    public int activateIdentity(String collaboratorId, String authenticationEmail) {
        return jdbcTemplate.update("""
                ...
                SET status = 'ATIVA',
                    email_verificado_em = COALESCE(
                        identity.email_verificado_em, clock_timestamp()),
                    versao_linha = identity.versao_linha + 1
                FROM colaborador
                WHERE colaborador.id = identity.colaborador_id
