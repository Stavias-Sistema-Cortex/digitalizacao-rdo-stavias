...
public class OperationalGraphProjector {
    ...
    public GraphProjectionBatch project(CommittedOperationalEvent event) {
        ...
        String sourceWorksiteId = event.sourceWorksiteId();
        ...
        event = new CommittedOperationalEvent(
                ...
                canonicalRelatedEntities(event.relatedEntities(), sourceWorksiteId),
                event.occurredAt(),
                canonicalPayload(event, sourceWorksiteId),
                sourceWorksiteId
        );
    ...
    private static Map<String, Object> canonicalPayload(
        ...
        safe.remove("worksiteId");
        safe.remove("obraId");
        if (sourceWorksiteId != null) {
            safe.put("worksiteId", sourceWorksiteId);
            if (event.payload().containsKey("obraId")) {
                safe.put("obraId", sourceWorksiteId);
            }
        }
        return safe;
    }

    private static List<CommittedOperationalEvent.EntityRef> canonicalRelatedEntities(
            List<CommittedOperationalEvent.EntityRef> relatedEntities,
            String sourceWorksiteId
    ) {
        Map<String, CommittedOperationalEvent.EntityRef> canonical = new LinkedHashMap<>();
        for (CommittedOperationalEvent.EntityRef reference : relatedEntities) {
            String normalizedType = normalizeType(reference.type());
            if ("WORKSITE".equals(normalizedType)) {
                if (sourceWorksiteId == null || !sourceWorksiteId.equals(reference.id())) {
                    continue;
                }
            }
            canonical.putIfAbsent(
                    normalizedType + "\u0000" + reference.id(),
                    reference
            );
        }
        if (sourceWorksiteId != null) {
            CommittedOperationalEvent.EntityRef worksite =
                    new CommittedOperationalEvent.EntityRef("WORKSITE", sourceWorksiteId);
            canonical.putIfAbsent("WORKSITE\u0000" + sourceWorksiteId, worksite);
        }
    ...
    private static Map<String, Object> stateMetadata(CommittedOperationalEvent event) {
        Map<String, Object> metadata = new LinkedHashMap<>();
        metadata.put("commitId", event.commitId());
        copyIfPresent(event.payload(), metadata, "worksiteId", "obraId");
        return metadata;
    }
