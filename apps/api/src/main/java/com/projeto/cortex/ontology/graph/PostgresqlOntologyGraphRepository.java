...
public class PostgresqlOntologyGraphRepository implements OntologyGraphRepository {
    ...
    public void upsert(GraphProjectionBatch batch) {
        transactions.executeWithoutResult(status -> {
            ...
            batch.entities().forEach(entity -> {
                upsertEntity(entity);
                upsertEntityWorksiteSnapshot(entity, batch.sourceWorksiteId());
            });
            batch.relations().forEach(this::upsertRelation);
            batch.events().forEach(event -> upsertEvent(event, batch.sourceWorksiteId()));
            batch.states().forEach(state -> upsertState(state, batch.sourceWorksiteId()));
            batch.evidences().forEach(evidence ->
                    upsertEvidence(evidence, batch.sourceWorksiteId())
            );
    ...
    private void upsertEntityWorksiteSnapshot(
            GraphEntity entity,
        ...
        jdbc.update("""
                INSERT INTO ontology_entity_worksite_snapshots (
                    entity_id, source_worksite_id, canonical_name, description,
                    status, metadata_json, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?::jsonb, ?, ?)
                ON CONFLICT (entity_id, source_worksite_id) DO UPDATE SET
                    canonical_name = CASE WHEN ?
                        THEN EXCLUDED.canonical_name
                        ELSE ontology_entity_worksite_snapshots.canonical_name
                    END,
                    description = CASE WHEN ?
                        THEN EXCLUDED.description
                        ELSE ontology_entity_worksite_snapshots.description
                    END,
                    status = CASE WHEN ?
                        THEN EXCLUDED.status
                        ELSE ontology_entity_worksite_snapshots.status
                    END,
                    metadata_json =
                        ontology_entity_worksite_snapshots.metadata_json
    ...
    private void upsertEvent(GraphEvent event, String sourceWorksiteId) {
        jdbc.update("""
                ...
                ON CONFLICT (event_type, source_type, source_id, entity_id) DO UPDATE SET
                    ...
                    source_worksite_id = EXCLUDED.source_worksite_id
                """,
                event.id(),
                event.type(),
                event.entityId(),
    ...
    private void upsertState(GraphState state, String sourceWorksiteId) {
        ...
        Timestamp successorStart = jdbc.query("""
                ...
                WHERE entity_id = ?
                  AND state_type = ?
                  AND source_worksite_id IS NOT DISTINCT FROM ?
                  AND id <> ?
                  AND (
                      valid_from > ?
                ...
                : null,
                state.entityId(), state.type(), sourceWorksiteId, state.id(),
    ...
    private void upsertEvidence(GraphEvidence evidence, String sourceWorksiteId) {
        jdbc.update("""
