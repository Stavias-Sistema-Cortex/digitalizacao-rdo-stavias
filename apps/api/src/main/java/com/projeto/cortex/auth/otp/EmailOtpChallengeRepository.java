...
/** Persistence boundary for single-use e-mail challenges. */
@Repository
public class EmailOtpChallengeRepository {
    ...
    public Instant create(
        ...
        if (inserted != 1) {
            throw new IllegalStateException(
                    "Desafio de autenticação não persistido."
            );
        }
    ...
    public Optional<LockedChallenge> lockForVerification(
        ...
        List<LockedChallenge> rows = jdbcTemplate.query("""
                SELECT
                    challenge.id,
                    challenge.colaborador_id,
                    challenge.codigo_digest,
                    challenge.expira_em,
                    challenge.tentativas,
                    challenge.max_tentativas,
        ...
        if (rows.size() > 1) {
            throw new IllegalStateException(
                    "Desafio de autenticação ambíguo."
            );
        }
    ...
    public Optional<DeliveryState> deliveryState(String challengeId) {
        ...
        if (rows.size() > 1) {
            throw new IllegalStateException(
                    "Estado de entrega de autenticação ambíguo."
            );
        }
        return rows.stream().findFirst();
    }
    ...
    public int consume(
        ...
        return jdbcTemplate.update("""
                ...
                SET status = 'CONSUMIDO',
                    consumido_em = CURRENT_TIMESTAMP(6)
                WHERE id = ?
                  AND colaborador_id = ?
                  AND status = 'PENDENTE'
                  AND expira_em > CURRENT_TIMESTAMP(6)
