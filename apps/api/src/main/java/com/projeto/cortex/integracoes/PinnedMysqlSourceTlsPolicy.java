...
final class PinnedMysqlSourceTlsPolicy {
    ...
    private final String pinnedTrustStoreUrl;
    private final String redactedFailure;
    private final KeyStoreLoader keyStoreLoader;

    PinnedMysqlSourceTlsPolicy(
            Path repositoryRoot,
            Path trustStorePath,
            String pinnedTrustStoreUrl,
            String redactedFailure
    ) {
        this(
                repositoryRoot,
                trustStorePath,
                pinnedTrustStoreUrl,
                redactedFailure,
                PinnedMysqlSourceTlsPolicy::loadPkcs12
        );
    }
    ...
    PinnedMysqlSourceTlsPolicy(
        ...
        this.trustStorePath = trustStorePath.toAbsolutePath().normalize();
        this.pinnedTrustStoreUrl = pinnedTrustStoreUrl;
        this.redactedFailure = redactedFailure;
    ...
    private void validateUnredacted(String jdbcUrl) throws Exception {
        ...
        if (!pinnedTrustStoreUrl.equals(properties.get(TRUST_STORE_URL))) {
            throw failure();
        }
        if (!PKCS12.equalsIgnoreCase(properties.get(TRUST_STORE_TYPE))) {
            throw failure();
        }
    ...
    private Map<String, String> decodedUniqueQuery(String jdbcUrl) {
        ...
        for (String parameter : query.split("&", -1)) {
            ...
            String decodedName = URLDecoder.decode(
                    parameter.substring(0, separator).strip(),
                    StandardCharsets.UTF_8
            );
            String decodedValue = URLDecoder.decode(
                    parameter.substring(separator + 1).strip(),
                    StandardCharsets.UTF_8
            );
    ...
    private void validatePinnedTrustStore(char[] password) throws Exception {
        ...
        KeyStore trustStore = keyStoreLoader.load(trustStorePath, password);
    ...
    private IllegalStateException failure() {
        return new IllegalStateException(redactedFailure);
    }
